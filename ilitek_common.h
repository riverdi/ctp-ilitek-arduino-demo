...
#define LOG_LEVEL ( ILITEK_DEBUG_LOG_LEVEL )

extern void ilitek_print ( const char * fmt, ... );

#define DBG_LVL(level, fmt, arg...) do {\
	if (level <= LOG_LEVEL) {\
		if (level == ILITEK_ERR_LOG_LEVEL) {\
			ilitek_print(" %s ERR  line = %d %s : " fmt, "ILITEK", __LINE__, __func__, ##arg);\
		} else if (level == ILITEK_INFO_LOG_LEVEL) {\
			ilitek_print(" %s INFO line = %d %s : " fmt, "ILITEK", __LINE__, __func__, ##arg);\
		} else if (level == ILITEK_DEBUG_LOG_LEVEL) {\
			ilitek_print(" %s DEBUG line = %d %s : " fmt, "ILITEK", __LINE__, __func__, ##arg);\
		} \
	} \
} while (0)

#define LOG_ERR(fmt, arg...) DBG_LVL(ILITEK_ERR_LOG_LEVEL, fmt, ##arg)
#define LOG_INFO(fmt, arg...) DBG_LVL(ILITEK_INFO_LOG_LEVEL, fmt, ##arg)
#define LOG_DEBUG(fmt, arg...) DBG_LVL(ILITEK_DEBUG_LOG_LEVEL, fmt, ##arg)
